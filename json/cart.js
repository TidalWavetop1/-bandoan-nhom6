...
function showOrderHistory() {
    ...
    if (orderHistory.length === 0) {
        orderHistoryContainer.innerHTML = '<p>Bạn chưa có lịch sử mua hàng.</p>';
    } else {
        orderHistory.forEach(order => {
            ...
            orderElement.innerHTML = `
                ...
                <ul>
                    ${order.items.map(item => `<li>${item.name} - SL: ${item.soluong} - Giá: ${item.price}</li>`).join('')}
                </ul>
            `;
    ...
    orderHistoryOverlay.style.display = 'flex';
}
...
function showOrderSummary(order) {
    ...
    orderSummaryContent.innerHTML = `
        ...
        <button onclick="confirmOrderSummary()">Xác nhận</button>
    `;
...
document.getElementById('shipping-form').addEventListener('submit', function (event) {
    ...
    const order = {
        ...
        totalPrice: document.getElementById('shipping-total-price').innerText
    };
