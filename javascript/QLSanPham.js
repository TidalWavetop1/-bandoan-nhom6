...
function openEditProductModal(idproduct) {
    ...
    if (product) {
        ...
        if (product.img && product.img !== 'image/no product.jpg') {
...
function removeCurrentImage() {
    ...
    if (product) {
        product.img = 'image/no product.jpg'; // Đánh dấu ảnh đã bị xóa
...
function updateProduct() {
    ...
    if(flag) {
        if (imgElement) {
            ...
            const img = existingProduct.img ? existingProduct.img : 'image/no product.jpg';
            updateProductData(name, price, quantity, type, description, img);
        }
    }
}
